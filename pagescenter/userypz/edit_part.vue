			...
			<view class="cu-item arrow">
				<view class="flex align-center justify-between" style="width: 100%;">
					...
					<view class="other flex justify-end align-center"
						...
						<input type="number" v-model="max_salary" maxlength="2" placeholder="请输入"
			...
			<view class="cu-item arrow">
				<view class="mt5" style="width: 100%;">
					...
					<view class=" text-df" style="width: 100%;">
						<input type="number" maxlength="11" placeholder="请输入" @blur="leavePhone"
				...
				<view class=" margin-top  " style=" ">
					...
					<view class="cu-form-group">
						<view class="grid  grid-square flex-sub">
							<view class="bg-img" v-if="videoImg&&videoImg.length > 0">
							...
							<view class="solids" @tap="ChooseImage" v-if="videoImg&&videoImg.length == 0">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.aid = option.aid
			this.other_user_id = option.other_user_id
		...
		computed: {
			totalMoney() {
				return ((this.zhaopin_number * (this.price_butie * 100) / 100) + ((this.zhaopin_number * (this
					.price_butie * 100) * this.part_job_info.part_job_percentage) / 10000)).toFixed(2);
			}
			// commission{
			// 	return (((this.zhaopin_number * (this.price_butie * 100)*this.part_job_info.part_job_percentage) / 10000)).toFixed(2);
			// }
		},
		onUnload() {
			uni.$off('video');
		...
		methods: {
			...
			get_detail() {
				...
				this.$Request
					.post(this.$api.operation.part_job_detail, {
						...
						user_id: this.user_id, //发布兼职任务的用户id,
						type: 4, //1：个人中心 2：首页详情 3：我的接单详情页 4:编辑网约工详情
					})
					.then(res => {
						if (res.code == 1) {
							uni.hideLoading()
							this.plant_detail = res.data;
							this.zhiwei_id = res.data.type
							if (res.data.type == 2) {
								this.zhiwei = '临时工'
							} else if (res.data.type == 1) {
								this.zhiwei = '网络兼职'
							}
							this.busort_id = res.data.settlement_method //结算方式id
							// console.log(this.busort_id,this.price_sort[this.busort_id-1],'-----------');
							if (this.busort_id && this.price_sort) {
								this.busort = this.price_sort[(this.busort_id) - 1].label //结算方式
							}
							this.service_time = res.data.service_time //服务时间
							this.price_butie = res.data.commission //佣金
							this.zhaopin_number = res.data.num
							this.min_salary = res.data.age_arr[0]
							this.max_salary = res.data.age_arr[1]
							this.sex = this.sex_sort[res.data.sex].label
							this.sex_id = res.data.sex
							this.phone = res.data.phone
							if (res.data.video != '') {
								this.video = res.data.video[0].video
								this.videoImg = res.data.video[0].url
							}
							...
							this.enterpriseIntroduction = res.data.remarks || ''
			...
			gopay() {
				...
				let formData = {
					aid: this.aid,
					user_id: this.other_user_id, //发布人id
					...
					// commission:(this.zhaopin_number * (this.price_butie * 100)/100).toFixed(2),//佣金
					commission: this.price_butie, //服务佣金
					commission_money: this.totalMoney, //总佣金加服务佣金
					remarks: this.enterpriseIntroduction, //备注
					lon: this.lon,
					lat: this.lat,
					is_pay: this.plant_detail.is_pay
				};
				if (this.isopen == false) {
					return;
				}
				...
				this.$Request.post(this.$api.company.edit_part, formData).then(res => {
					if (res.code == 1) {
						if (this.voucher == 1) {
							uni.$emit('updateOrderDetail', {})
							uni.navigateBack({})
						...
						if (res.data.is_pay == 1) {
							...
							uni.redirectTo({
								url: '../zhaopinzhe/fabu?title=fabu&order_sn=' + res.data.order_sn +
									'&price=' + res.data.payable_money + '&commission=' + res.data
									.commisson,
							});
						}
