	...
	<view class="">
		...
		<view class="card_item" @tap='chooseImg(index)' v-for="(item,index) in img" :key='index'>
			<view class="margin-tb-sm">
				<image class="bg_img" :src="item.pic||IMG_URL+'static/miniwechat/cardbg.png'" mode="aspectFill" @tap.stop='yulan(item.pic,index)'></image>
				<view class="btn_chuan" v-show="item.pic==''">
					<view class="btn_center flex align-center justify-center">
						...
						<view class="" style="flex: 1 1 80%;">{{item.content}}</view>
					</view>
				</view>
			</view>
		</view>
		...
		<!-- 底部按钮 -->
		<view class="btn_next bg_deep" @tap="sub" style="height: 88upx;margin-top: 20upx; margin-bottom: 40upx;">完成</view>
	</view>
</template>
		...
		methods: {
			...
			getPrice() { //获取保证金
				this.$Request.post(this.$api.company.getPrice).then(res => {
					if (res.code == '1') {
						if (res.data) {
							this.totalMoney = res.data.manpower_company_bond_money
						}
					}
				});
			},
...
<style lang="scss">
	...
	.card_item:nth-child(3) .btn_chuan {
		// height: 116upx;
		border-radius: 100upx;
	}

	.card_item:nth-child(3) .btn_center view {
		width: 69%;
	}
</style>
