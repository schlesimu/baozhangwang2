	...
	<view class=" padding">
		<form @submit="formSubmit" class=" ">
			<view class="head_list cu-list menu sm-border">
				...
				<view class="list_item flex align-center justify-between">
					<view class="color6 f28">是否发送简历</view>
					<view class="flex align-center">
						<view class="mr10">是</view>
						<!-- <switch checked @change="switch1Change" :color='color' style="transform:scale(0.7)"/> -->
						<switch :checked="checked" :class="checked ? 'checked' : '' " class="deepBlue" @change="switch1Change" style="transform:scale(0.7)" />
					</view>
				</view>
				<view>
					<button class="btn_next bg_deep" style="height: 88upx;margin-bottom: 80upx;" form-type="submit">确认报名</button>
				</view>
			</view>
		</form>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.red_envelope_type = option.red_envelope_type
		...
		methods: {
			...
			formSubmit: function(e) {
				...
				this.$Request.post(this.$api.common.singupPlay, {
					...
					username: this.nickname, //姓名
					sex: this.sexIndex,
					phone: this.phone, //手机号
					card_front_id: this.img[0].id, //正面
					card_side_id: this.img[1].id, //反面
					is_uplode: this.isDefault, //是否提交简历 1：是 0：否
				}).then(res => {
					if (res.code == 1) {
						if (this.red_envelope_type == 0) {
							...
							uni.redirectTo({
								url: 'sign_success?company_id=' + this.company_id + '&job_id=' + this.job_id + '&delivery_id=' + res.data.delivery_id
							})
						}
